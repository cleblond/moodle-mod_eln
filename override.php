...
eln_override_lock($pageversion->pageid);

$redirpage = optional_param('redirpage', '', PARAM_ALPHA);

if ($redirpage != '') {
    redirect($redirpage.'.php?'.eln_display_wiki_parameters($pagename, $subwiki, $cm, OUWIKI_PARAMS_URL), '', 0);
} else {
    redirect('edit.php?'.eln_display_wiki_parameters($pagename, $subwiki, $cm, OUWIKI_PARAMS_URL), '', 0);
}
