...
class mod_eln_annotate_form extends moodleform {
    ...
    public function definition() {
        ...
        if (count($annotations != 0)) {
            usort($annotations, array('mod_eln_annotate_form', 'eln_internal_position_sort'));
            $editnumber = 1;
        ...
        if ($orphaned) {
            $mform->addElement('checkbox', 'deleteorphaned', get_string('deleteorphanedannotations', 'eln'));
        }

        if ($canlock) {
            $mform->addElement('checkbox', 'lockediting', get_string('lockediting', 'eln'));
            if (eln_is_page_editing_locked($pageid)) {
    ...
    private function eln_internal_position_sort($a, $b) {
