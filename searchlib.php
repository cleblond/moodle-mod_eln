...
class eln_document {
    ...
    public static function delete_module_instance_data($cm) {
        ...
        $DB->delete_records_select('eln_occurrences',
                'documentid IN (SELECT id FROM {eln_documents} WHERE ' .
                $where . ')', $wherearray);
        $DB->delete_records_select('eln_documents',
                $where, $wherearray);
    }
    ...
    public function find() {
        ...
        $this->id = $DB->get_field_select('eln_documents', 'id',
                $where, $wherearray);
    ...
    public function update($title, $content, $timemodified=null, $timeexpires=null,
        ...
        if (!$this->find()) {
            ...
            $this->id = $DB->insert_record('eln_documents', $this);
        }
        ...
        if ($timemodified || $timeexpires) {
            ...
            $DB->update_record('eln_documents', $update);
        }

        // Delete existing words
        $DB->delete_records('eln_occurrences',
    ...
    private function internal_add_words($title,$content) {
        ...
        $dbwords = $DB->get_records_select('eln_words',
            ...
            if (count($missingwords)>0) {
                foreach ($sequences as $sequenceindex => $sequenceselect) {
                    ...
                    if (!pg_copy_from($DB->get_pgsql_connection_id(),
                            $DB->get_prefix() . 'eln_words',
                            $data)) {
                        throw new dml_exception(
                                'fastinserterror', 'mod_eln');
                    }
                }
            }
        } else {
        ...
        if ($fastpg && count($wordset)>0) {
            ...
            if (!pg_copy_from($DB->get_pgsql_connection_id(),
                    $DB->get_prefix() . 'eln_occurrences', $data)) {
                throw new dml_exception(
                        'fastinserterror', 'mod_eln');
            }
        } else {
            ...
            foreach ($wordset as $word=>$count) {
                ...
                $score = ($bodycount<15 ? $bodycount : 15) +
                        ($titlecount<15 ? $titlecount*16 : 15*16);
                $DB->execute('INSERT INTO {eln_occurrences}' .
    ...
    private static function replace_with_underline($pattern, $text) {
        return preg_replace_callback($pattern,
                array('eln_document','internal_replace_callback'),
                $text);
    }
    ...
    public function wipe_document($id) {
        ...
        $DB->delete_records('eln_occurrences',
                array('documentid' => $id));
        $DB->delete_records('eln_documents',
                array('id' => $id));
    }
}
...
class eln_search {
    ...
    public function set_query($query) {
        ...
        $words = eln_document::split_words($query, true);
        $currentquote = array();
        $sign = false;
        $inquote = false;
        foreach ($words as $word) {
     ...
     *   eln_search::NONE to return only results that have no group
     * @param bool $ornone If true, also returns results that have no group
     *   (ignored if first parameter is eln_search::NONE or null)
     */
     ...
     * @see eln_search::get_group_exceptions
     ...
     *   eln_search::NONE to return only results that have no user
     * @param bool $ornone If true, also returns results that have no user
     *   (ignored if first parameter is eln_search::NONE or 0)
     */
    ...
    private function internal_translate_words() {
        ...
        $words = $DB->get_records_select('eln_words',
     ...
     *   ->documentid, ->totalscore, all fields from eln_documents,
        ...
        foreach ($this->terms as $term) {
            foreach ($term->ids as $id) {
                ...
                if ($join==0) {
                    ...
                    // parameter, because
                    $from .= "
INNER JOIN {eln_occurrences} $alias
    ON $alias.documentid=o0.documentid AND $alias.wordid=?";
        ...
        $maxterms = $CFG->eln_maxterms;
        if ($join > $maxterms) {
            $referer = $_SERVER['HTTP_REFERER'];
            if (!$referer) {
                $referer = ''; // Use default
            }
            print_error('toomanyterms', 'eln', $referer, $maxterms);
        }
        foreach ($this->negativeterms as $term) {
            if (count($term->ids)==1) {
                $alias = "o$join";
                $from .= "
LEFT JOIN {eln_occurrences} $alias
    ON $alias.documentid=o0.documentid AND $alias.wordid=?";
...
SELECT
    ...
    g.name AS groupname
FROM $from
INNER JOIN {eln_documents} d ON d.id=o0.documentid
...
ORDER BY totalscore DESC, o0.documentid";
    ...
    private function internal_filter($results,$desired) {
        ...
        foreach ($results as $result) {
            ...
            $textcontent = eln_document::strip_xhtml($page->content);
    ...
    public function display_results($baseurl, $title=null) {
        ...
        if ($title === null) {
            $title = get_string('searchresultsfor', 'mod_eln',
                    s($query));
        }
    ...
    public static function format_results($results, $title, $number=1,
        ...
        if (!$results->success) {
            if ($results->problemword === null){
                $out .= '<p>' . get_string('nowordsinquery', 'mod_eln') .
                        '</p>';
            } else {
                $out .= '<p>' . get_string('resultsfail', 'mod_eln',
                        $results->problemword) . '</p>';
            }
        }  else {
            if ($prevlink) {
                $out .= '<p>' . link_arrow_left(
                    get_string('previousresults', 'mod_eln', $prevrange),
            ...
            if ($nextlink) {
                $out .= '<p>' . link_arrow_right(
                        get_string('findmoreresults', 'mod_eln'),
    ...
    public static function require_remote_access() {
        $addresslist = get_config('mod_eln', 'remote');
        if (!preg_match('/^[0-9.]+(,\s*[0-9.]+)*$/', $addresslist)) {
                throw new moodle_exception('remotewrong', 'mod_eln');
        }
        ...
        throw new moodle_exception('remotenoaccess', 'mod_eln');
    }
