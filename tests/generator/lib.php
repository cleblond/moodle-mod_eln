...
/**
 * eln module data generator class
 *
 * @package    mod_eln
 * @copyright  2014 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_eln_generator extends testing_module_generator {
    ...
    public function create_content($instance, $record = array()) {
        ...
        $cm = get_coursemodule_from_instance('eln', $instance->id);
        $context = context_module::instance($cm->id);
        // Setup subwiki.
        if (!isset($record['subwiki'])) {
            // Create a new sub wiki object for current user.
            $subwiki = eln_get_subwiki($instance->course, $instance, $cm, $context, 0, $USER->id, true);
        } else {
        ...
        if (isset($record['newversion'])) {
            ...
            return eln_save_new_version($instance->course, $cm, $instance, $subwiki, $newverinfo->pagename,
            ...
            eln_get_current_page($subwiki, $newpageinfo->pagename, OUWIKI_GETPAGE_CREATE);
            ...
            eln_create_new_page($instance->course, $cm, $instance, $subwiki, $newpageinfo->pagename,
                    $newpageinfo->newpagename, $newpageinfo->content, $newpageinfo->formdata);
            return eln_get_current_page($subwiki, $newpageinfo->newpagename);
        }
    }
