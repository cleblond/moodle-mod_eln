...
// Get history
$changes = eln_get_page_history($pageversion->pageid, false, 0, OUWIKI_FEEDSIZE);
...
$pagetitle = $pageversion->title === '' ? get_string('startpage', 'eln') :
...
$feeddescription = get_string('feeddescriptionhistory', 'eln');

// Domain name, used for IDs (we assume this is owned by site operator in 2007)
$domainname = preg_replace('/^.*\/\/(www\.)?(.*?)\/.*$/', '$2', $CFG->wwwroot);
$id = 'tag:'.$domainname.',2007:eln/'.$eln->id.'/wikihistory/changes/'.$pageversion->pageid;

$wikiparams = eln_display_wiki_parameters($pagename, $subwiki, $cm);
$pagelink = $CFG->wwwroot .'/mod/eln/history.php?'.$wikiparams;
...
$pageparams = eln_display_wiki_parameters($pagename, $subwiki, $cm);
...
foreach ($changes as $change) {
    ...
    if ($change->versionid == $pageversion->versionid) {
        ...
        $itemlink = $CFG->wwwroot.'/mod/eln/viewold.php?'.$pageparams.'&amp;version='.$change->versionid;
    }
    $itemtitle = $eln->name.' - '.$pagetitle.' ('.eln_nice_date($change->timecreated).')';
    ...
    $itemdescription = get_string('feeditemdescriptionnodate', 'eln', $a);
    if ($rss) {
...
<item>
  ...
  <guid>'.$CFG->wwwroot.'/mod/eln/history.php?'.$pageparams.'#v'.$change->versionid.'</guid>
</item>';
    } else {
        print '
<entry>
