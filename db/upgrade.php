...
function xmldb_eln_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2008100600) {
        ...
        // Define field deletedat to be added to eln_versions.
        $table = new xmldb_table('eln_versions');
        ...
        upgrade_mod_savepoint(true, 2008100600, 'eln');
    }

    if ($oldversion < 2009120801) {
        // Launch create table for eln_annotations - if it does not already exist (extra precaution).
        $table = new xmldb_table('eln_annotations');
        if (!$dbman->table_exists($table)) {
            $dbman->install_one_table_from_xmldb_file($CFG->dirroot.'/mod/eln/db/install.xml', 'eln_annotations');
        }

        // Define field locked to be added to eln_pages.
        $table = new xmldb_table('eln_pages');
        ...
        upgrade_mod_savepoint(true, 2009120801, 'eln');
    }

    if ($oldversion < 2010022300) {

        // Define field locked to be added to eln_pages.
        $table = new xmldb_table('eln');
        ...
        upgrade_mod_savepoint(true, 2010022300, 'eln');
    }

    if ($oldversion < 2010122001) {

        // Drop the old comments table
        $table = new xmldb_table('eln_comments');
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Drop the old commenting field in eln
        $table2 = new xmldb_table('eln');
        $field = new xmldb_field('commenting');
        ...
        // Define field annotation to be added to eln
        ...
        if (!empty($rs)) {
            ...
            $update_sql = 'UPDATE {eln} SET annotation = 1 WHERE id '.$usql;
        ...
        // Define field introformat to be added to eln
        ...
        // Define field introformat to be added to eln
        $table3 = new xmldb_table('eln_versions');
        ...
        if ($CFG->texteditors !== 'textarea') {
            // introformat
            $rs = $DB->get_recordset('eln', array('introformat' => FORMAT_MOODLE), '', 'id,intro,introformat');
            foreach ($rs as $r) {
                $r->intro       = text_to_html($r->intro, false, false, true);
                $r->introformat = FORMAT_HTML;
                $DB->update_record('eln', $r);
                upgrade_set_timeout();
            }
            $rs->close();

            // xhtmlformat
            $rs = $DB->get_recordset('eln_versions', array('xhtmlformat' => FORMAT_MOODLE), '', 'id,xhtml,xhtmlformat');
            foreach ($rs as $r) {
                $r->xhtml       = text_to_html($r->xhtml, false, false, true);
                $r->xhtmlformat = FORMAT_MOODLE;
                $DB->update_record('eln_versions', $r);
                upgrade_set_timeout();
            }
            $rs->close();
        }

        upgrade_mod_savepoint(true, 2010122001, 'eln');
    }

    if ($oldversion < 2011031800) {
        upgrade_mod_savepoint(true, 2011031800, 'eln');
    }

    if ($oldversion < 2011060100) {

        // Define field enablewordcount to be added to eln
        $table = new xmldb_table('eln');
        ...
        // Define field wordcount to be added to eln_versions
        $table = new xmldb_table('eln_versions');
        ...
        // we need to update all eln_versions wordcounts
        $count = $DB->count_records_sql('SELECT COUNT(*) FROM {eln_versions}');
        $rs = $DB->get_recordset_sql('SELECT id, xhtml FROM {eln_versions} ORDER BY id');
        if ($rs->valid()) {
            $pbar = new progress_bar('countwordselnversionsxhtml', 500, true);

            $i = 0;
            foreach ($rs as $entry) {
                $i++;
                upgrade_set_timeout(60); // set up timeout, may also abort execution
                $pbar->update($i, $count, "Counting words of eln version entries - $i/$count.");

                // retrieve wordcount
                require_once($CFG->dirroot.'/mod/eln/locallib.php');
                $wordcount = eln_count_words($entry->xhtml);
                $entry->wordcount = $wordcount;
                $DB->update_record('eln_versions', $entry);
            }
        }
        $rs->close();

        upgrade_mod_savepoint(true, 2011060100, 'eln');
    }

    if ($oldversion < 2011071300) {

        // Define field grade to be added to eln
        $table = new xmldb_table('eln');
        ...
        // we need to update all eln_versions scales
        $rs = $DB->get_recordset_sql('SELECT id FROM {eln} ORDER BY id');
        if ($rs->valid()) {
            foreach ($rs as $entry) {
                $entry->grade = 0;
                $DB->update_record('eln', $entry);
            }
        }
        $rs->close();

        upgrade_mod_savepoint(true, 2011071300, 'eln');
    }

    if ($oldversion < 2011072000) {

        // Define field firstversionid to be added to eln_pages
        $table = new xmldb_table('eln_pages');
        ...
        // Define field previousversionid to be added to eln_versions
        $table = new xmldb_table('eln_versions');
        ...
        // update all eln_versions previousversionids and eln_pages firstversionids
        // firstversionid
        $sql = 'SELECT v.pageid,
                    (SELECT MIN(id)
                        FROM {eln_versions} v3
        ...
        if ($rs->valid()) {
            foreach ($rs as $entry) {
                if (isset($entry->firstversionid)) {
                    $DB->set_field('eln_pages', 'firstversionid', $entry->firstversionid,
        ...
        $rs = $DB->get_recordset_sql($sql);
        if ($rs->valid()) {
            $pbar = new progress_bar('elnfirstandpreviousversions', 500, true);
            ...
            foreach ($rs as $entry) {
                ...
                if (isset($entry->previousversionid)) {
                    $DB->set_field('eln_versions', 'previousversionid',
        ...
        upgrade_mod_savepoint(true, 2011072000, 'eln');
    }
...
FROM
   {eln_pages} p
   LEFT JOIN {eln_pages} p2 ON p2.subwikiid = p.subwikiid AND p2.title IS NULL
...
WHERE
   p2.id IS NOT NULL");
        ...
        if ($ids) {
            ...
            $DB->execute("DELETE FROM {eln_pages} WHERE id $sql", $params);
        }

        // Set all the null values to blank
        $DB->execute("UPDATE {eln_pages} SET title='' WHERE title IS NULL");

        // Also in ousearch table if installed
        $table = new xmldb_table('local_ousearch_documents');
        if ($dbman->table_exists($table)) {
            $DB->execute("UPDATE {local_ousearch_documents} SET stringref='' WHERE stringref IS NULL AND plugin='mod_eln'");
        }

        // Changing nullability of field title on table eln_pages to not null
        $table = new xmldb_table('eln_pages');
        ...
        // Define index subwikiid-title (unique) to be added to eln_pages
        ...
        // eln savepoint reached
        upgrade_mod_savepoint(true, 2011102802, 'eln');
    }
    ...
    if ($oldversion < 2013060600) {
        ...
        $params[] = '<div class="eln_content"%';
        $params[] = 0;
        $sql = "select l.id as lid, l.tourl, v.id as vid, v.xhtml, p.id as pageid, p.subwikiid
            from {eln_links} l
            inner join {eln_versions} v on v.id = l.fromversionid
            inner join {eln_pages} p on v.id = p.currentversionid
            where ($likes)
        ...
        foreach ($links as $linkdetails) {
            ...
            if (!in_array($linkdetails->pageid, $pagesprocessed)) {
                ...
                for ($i = 0; $i < count($matches[0]); $i++) {
                    ...
                    if (strip_tags($text) == $text) {
                        ...
                        $newtag = str_replace("{$CFG->wwwroot}/view.php",
                            "{$CFG->wwwroot}/mod/eln/view.php", $tag);
                ...
                if ($newxhtml != $linkdetails->xhtml) {
                    ...
                    $DB->update_record('eln_versions', $update);
                }
            ...
            if (!isset($subwikipages[$linkdetails->subwikiid])) {
                ...
                $subwikipages[$linkdetails->subwikiid] = $DB->get_records('eln_pages',
            ...
            $DB->update_record('eln_links', $update);
        }
        $links = null;
        $subwikipages = null;
        // eln savepoint reached.
        upgrade_mod_savepoint(true, 2013060600, 'eln');
    }

    if ($oldversion < 2014012700) {

        // Define field allowimport to be added to eln.
        $table = new xmldb_table('eln');
        ...
        // Define field importversionid to be added to eln_versions.
        $table = new xmldb_table('eln_versions');
        ...
        upgrade_mod_savepoint(true, 2014012700, 'eln');
    }

    if ($oldversion < 2014031100) {

        // Define index eln_subwikis_unique_group (unique) to be added to eln_subwikis.
        $table = new xmldb_table('eln_subwikis');
        $index = new xmldb_index('eln_subwikis_unique_group', XMLDB_INDEX_UNIQUE, array('wikiid', 'groupid'));

        // Conditionally launch add index eln_subwikis_unique_group.
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }

        // Define index eln_subwikis_unique_user (unique) to be added to eln_subwikis.
        $table = new xmldb_table('eln_subwikis');
        $index = new xmldb_index('eln_subwikis_unique_user', XMLDB_INDEX_UNIQUE, array('wikiid', 'userid'));

        // Conditionally launch add index eln_subwikis_unique_user.
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }

        // Ouwiki savepoint reached.
        upgrade_mod_savepoint(true, 2014031100, 'eln');
    }
