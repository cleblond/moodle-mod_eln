...
/**
 * Import page content from other eln activities into this wiki.
 * @package mod
 * @subpackage eln
...
require($CFG->dirroot . '/mod/eln/import_form.php');

$pageparams = eln_display_wiki_parameters($pagename, $subwiki, $cm, OUWIKI_PARAMS_ARRAY);
...
$url = new moodle_url('/mod/eln/import.php', array('id' => $id, 'page' => $pagename));
$PAGE->set_url($url);
...
echo $elnoutput->eln_print_start($eln, $cm, $course, $subwiki, get_string('import', 'eln'), $context);

echo $OUTPUT->heading(get_string('import', 'eln'));
...
echo '<div class="eln_import eln_import_step' . $curstep . '" id="eln_belowtabs">';
...
if ($curstep == 1) {
    ...
    foreach ($courses as $listcourse) {
        ...
        $allwikis = $modinfo->get_instances_of('eln');
        unset($allwikis[$eln->id]);// Don't include current activity.
        ...
        foreach ($allwikis as $wikiact) {
            ...
            if (!$wikiact->uservisible ||
                    !has_capability('mod/eln:view', $wikicontext)) {
                continue;
            }
            ...
            if ($wikiinst->subwikis == OUWIKI_SUBWIKIS_SINGLE) {
                ...
                foreach ($groups as $group) {
                    ...
                    if ($wikisubwiki = $DB->get_record_select('eln_subwikis', 'wikiid = ?
                            AND groupid = ? AND userid IS NULL', array($wikiinst->id, $group->id), 'id')) {
                        if (eln_subwiki_content_exists($wikisubwiki->id)) {
                ...
                if (!has_capability('mod/eln:viewallindividuals', $wikicontext)) {
                    if (!has_capability('mod/eln:viewgroupindividuals', $wikicontext)) {
    ...
    if (!$wikisfound) {
        ...
        echo $OUTPUT->notification(get_string('unabletoimport', 'eln'));
        unset($pageparams['step']);
        echo $OUTPUT->continue_button(new moodle_url('/mod/eln/view.php', $pageparams));
    }
} else if ($curstep == 2) {
    ...
    eln_get_wikiinfo($selectedact, $selectedsubwiki, $selectedeln, $importedfromcourse);

    echo html_writer::tag('p', get_string('import_selectwiki', 'eln', $selectedact->get_formatted_name()));

    // Build page selector.
    $pages = '';
    $index = eln_get_subwiki_index($selectedsubwiki->id);
    eln_build_tree($index);
    // Print out in hierarchical form...
    $pages .= html_writer::start_tag('ul', array('class' => 'ouw_indextree'));
    $pages .= eln_tree_index('eln_display_wikiindex_page_in_index', reset($index)->pageid,
    ...
    foreach ($index as $indexitem) {
        if (count($indexitem->linksfrom) == 0 && $indexitem->title !== '') {
            ...
            $orphans .= eln_tree_index('eln_display_wikiindex_page_in_index',
                    $indexitem->pageid, $orphanindex, $selectedsubwiki, $selectedact, null, true);
        }
    }
    if (!empty($orphans)) {
        $pages .= $OUTPUT->heading(get_string('orphanpages', 'eln'), 3);
        $pages .= html_writer::start_tag('ul', array('class' => 'ouw_indextree'));
        $pages .= $orphans;
        $pages .= html_writer::end_tag('ul');
    }
    $PAGE->requires->yui_module('moodle-mod_eln-pageselector', 'M.mod_eln.pageselector.init', array($pagelist));
    // Prepare form parameters.
    $wikiinfo = array('importid' => $selectedact->id , 'subwikiid' => $selectedsubwiki->id);
    $pageparams['step']++;
    $pageparams['courseid'] = $importedfromcourse->id;
    $form = new mod_eln_import_pageselect_form(null, array('params' => array_merge($pageparams,
    ...
    if ($pages = optional_param('pages', null, PARAM_SEQUENCE)) {
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'page') === 0) {
                ...
                eln_add_linkedpages($pagelist, $index, $value);
            }
        }
        $pagelist = array_unique($pagelist);
    }
    echo $OUTPUT->heading(get_string('import_confirm', 'eln'), 3);
    // Setup confirm form.
    $confirmdata = array(
    ...
    foreach ($pagelist as $page) {
        ...
        if ($ipagename == '') {
            ...
            array_unshift($confirmdata['pages'], get_string('startpage', 'eln'));
        } else {
            $confirmdata['pages'][] = $ipagename;
        }
        // Check if this page already exists, and is locked.
        if ($pageexists = eln_get_current_page($subwiki, $ipagename, OUWIKI_GETPAGE_ACCEPTNOVERSION)) {
            $locked = false;
            if ($pageexists->locked || (eln_is_page_locked($pageexists->pageid) &&
                    !has_capability('mod/eln:overridelock', $context))) {
                $locked = true;
                if ($ipagename != '') {
    ...
    if (!$confirmdata['startselected'] && !empty($pagelist)) {
        ...
        $confirmdata['wikipages']['-1'] = get_string('import_confirm_linkfrom_newpage', 'eln');
        $thisindex = eln_get_subwiki_allpages_index($subwiki);
        foreach ($thisindex as $thispage) {
            if ($thispage->title == '') {
                $thispage->title = get_string('startpage', 'eln');
            }
            if ($thispage->locked) {
                // Prevent selection by adding a note that page is locked.
                $confirmdata['wikipages'][$thispage->pageid] = $thispage->title .
                '[' . get_string('import_lockedpage', 'eln') . ']';
            } else {
                $confirmdata['wikipages'][$thispage->pageid] = $thispage->title;
            }
        }
        if (empty($thisindex)) {
            ...
            $confirmdata['wikipages'][0] = get_string('startpage', 'eln');
        }
    }
    $wikiinfo = array('importid' => $selectedact->id , 'subwikiid' => $selectedsubwiki->id,
    ...
    $pageparams['step']++;
    $pageparams['courseid'] = $importedfromcourse->id;
    $form = new mod_eln_import_confirm_form(null, array('params' => array_merge($pageparams, $wikiinfo),
    ...
    $index = eln_get_subwiki_allpages_index($selectedsubwiki);
    ...
    echo $OUTPUT->heading(get_string('import_process', 'eln'), 3);
    try {
        ...
        } else if ($pageexists = eln_get_current_page($subwiki, '', OUWIKI_GETPAGE_CREATE)) {
        ...
        if ($startpagemerge == 1) {
            if ($currentpageexists = eln_get_current_page($subwiki, get_string('importedstartpage', 'eln'),
                    OUWIKI_GETPAGE_ACCEPTNOVERSION)) {
                $checkpagelist[] = $currentpageexists->pageid;
            }
        }
        if ($linkfrom == -1) {
            if ($currentpageexists = eln_get_current_page($subwiki, get_string('importedpages', 'eln'),
                        OUWIKI_GETPAGE_ACCEPTNOVERSION)) {
                $checkpagelist[] = $currentpageexists->pageid;
            }
        }
        ...
        foreach ($checkpagelist as $page) {
            if (!isset($index[$page])) {
                $ipagename = $DB->get_field('eln_pages', 'title', array('id' => $page), MUST_EXIST);
            } else {
                $ipagename = $index[$page]->title;
            }
            if ($ipagename == '' && $startpagemerge == 1) {
                // If start page exists then set imported start page in conflicts array.
                if (eln_get_current_page($subwiki, $ipagename, OUWIKI_GETPAGE_ACCEPTNOVERSION)) {
            ...
            // Check if this page already exists, and is locked.
            if ($currentpageexists = eln_get_current_page($subwiki, $ipagename, OUWIKI_GETPAGE_ACCEPTNOVERSION)) {
                $conflicts[$page] = $ipagename;
                if (has_capability('mod/eln:overridelock', $context)) {
                    eln_override_lock($currentpageexists->pageid);
                }
                list ($lock, $by) = eln_obtain_lock($eln, $currentpageexists->pageid);
                if ($currentpageexists->locked) {
                    $lock = false;// No one can edit.
                }
                if (!$lock) {
                    throw new moodle_exception('import_process_locked', 'eln');
                }
            }
        }
        ...
        $pbar = new progress_bar('mod_eln_import', 500, true);
        for ($len = count($pagelist), $a = 0; $a < $len; $a++) {
            ...
            if (in_array($page, array_keys($conflicts))) {
                ...
                if ($pageinfo->title == '' && $startpagemerge == 1) {
                    ...
                    $importedstartpage = get_string('importedstartpage', 'eln');
                    $newpage = true;
                    // See whether an 'Imported start page' already exists in the target wiki.
                    if ($importedstartpageexists = eln_get_current_page($subwiki, $importedstartpage, OUWIKI_GETPAGE_CREATE)) {
                        ...
                        $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, $importedstartpageexists->title,
                        ...
                        $updatepagelist[] = get_string('startpage', 'eln');
                    }
                } else {
                    ...
                    if ($conflictmerge == 0 || ($pageinfo->title == '' && $startpagemerge == 0)) {
                        ...
                        if (!$currentpageexists = eln_get_current_page($subwiki, $pageinfo->title,
                        ...
                        // Save new version of existing page.
                        $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, $currentpageexists->title,
                        ...
                        // Save new version of existing page.
                        $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, $pageinfo->title, $importedcontent,
                ...
                $pageversion = eln_get_current_page($subwiki, $pageinfo->title, OUWIKI_GETPAGE_CREATE);
                $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, $pageinfo->title, $pageinfo->xhtml,
                        -1, -1,  -1, null, null, null,  $pageinfo->versionid);
            ...
            // Add any files.
            if ($oldfiles = $fs->get_area_files($modcontext->id, 'mod_eln', 'content', $prevversion)) {
            ...
            // Add any attachments.
            if ($oldfiles = $fs->get_area_files($modcontext->id, 'mod_eln', 'attachment', $prevversion)) {
                foreach ($oldfiles as $oldfile) {
                    try {
                        // Copy this file to the version record.
                        $fs->create_file_from_storedfile(array(
            ...
            $pbar->update($a + 1, count($pagelist), get_string('import_process', 'eln'));
        ...
        if (!is_null($linkfrom)) {
            ...
            if ($linkfrom == 0) {
                ...
                $updatepagelist[] = get_string('startpage', 'eln');
                $pageversion = eln_get_current_page($subwiki, null, OUWIKI_GETPAGE_CREATE);
                //  Add links into selected page content.
                $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, '', $mergexhtml);
            } else if ($linkfrom == -1) {
                ...
                $importedpages = get_string('importedpages', 'eln');
                // Create/update(merge) 'Imported pages'.
                $linkfrompage = eln_get_current_page($subwiki, $importedpages, OUWIKI_GETPAGE_CREATE);
                ...
                $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, $linkfrompage->title, $linkfrompage->xhtml);
                // Add page names to $newpagelist|$updatepagelist array as appropriate.
                if ($newpage) {
                    ...
                    eln_save_new_version($course, $cm, $eln, $subwiki, $title, $xhtmlcontent);
                } else {
                ...
                if (!$pageinfo = $DB->get_record('eln_pages', array('id' => $linkfrom))) {
                    throw new moodle_exception('Error. Page to import into missing.');
                }
                // Store page name so we can go there.
                $linkfrom = $pageinfo->title;
                // Add links into selected page content.
                $linkfrompage = eln_get_current_page($subwiki, $pageinfo->title, OUWIKI_GETPAGE_CREATE);
                $linkfrompageid = $linkfrompage->pageid;
                // Merge links xhtml into import into page xhtml content.
                $linkfrompage->xhtml = $linkfrompage->xhtml . $mergexhtml;
                $versionid = eln_save_new_version($course, $cm, $eln, $subwiki, $linkfrompage->title, $linkfrompage->xhtml);
        ...
        echo $OUTPUT->heading(get_string('import_process_summary', 'eln'), 3);
        echo html_writer::tag('p', $warned ? get_string('import_process_summary_warn', 'eln') :
                get_string('import_process_summary_success', 'eln'));
        if ($startpagelocked) {
            // Start page locked.
            echo html_writer::div(get_string('import_process_startpage_locked', 'eln'));
        }
        if ($pagelocked) {
            // Page imported into locked.
            echo html_writer::div(get_string('import_process_locked', 'eln'));
        }

        // Show pages imported.
        echo html_writer::start_div();
        echo get_string('import_process_summary_imported', 'eln');
        echo html_writer::alist(array_unique($newpagelist));
        echo html_writer::end_div();
        if (!empty($updatepagelist)) {
            // Show pages updated.
            echo html_writer::start_div();
            echo get_string('import_process_summary_updated', 'eln');
        ...
        $wikiinfo = array('importid' => $selectedact->id , 'subwikiid' => $selectedsubwiki->id,
                ...
                'sesskey' => sesskey());
        $continue = new moodle_url('/mod/eln/import.php', array_merge($pageparams, $wikiinfo));
        $pageparams['step']++;
        $pageparams['cancel'] = get_string('cancel');
        $cancel = new moodle_url('/mod/eln/import.php', array_merge($pageparams, $wikiinfo));
        echo $OUTPUT->confirm($e->getMessage(), $cancel, new moodle_url('/mod/eln/view.php', $pageparams));
 ...
 */
function eln_add_linkedpages(&$pagelist, &$index, $value) {
    if (!in_array($value, $pagelist) && in_array($value, array_keys($index))) {
        ...
        foreach ($newpages as $newvalue) {
            eln_add_linkedpages($pagelist, $index, $newvalue);
        }
    }
}
