...
/**
 * Define all the backup steps that will be used by the backup_eln_activity_task
 */

/**
 * Define the complete eln structure for backup, with file and id annotations
 */
class backup_eln_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $eln = new backup_nested_element('eln', array('id'), $els);
        ...
        // Define sources
        $eln->set_source_table('eln', array('id' => backup::VAR_ACTIVITYID));

        // All these source definitions only happen if we are including user info
        if ($userinfo) {
            $subwiki->set_source_table('eln_subwikis', array('wikiid' => backup::VAR_PARENTID));

            $page->set_source_table('eln_pages', array('subwikiid' => backup::VAR_PARENTID));

            $version->set_source_table('eln_versions', array('pageid' => backup::VAR_PARENTID));

            $link->set_source_table('eln_links', array('fromversionid' => backup::VAR_PARENTID));

            $annotation->set_source_table('eln_annotations', array('pageid' => backup::VAR_PARENTID));
        }
        ...
        $eln->annotate_files('mod_eln', 'intro', null); // This file area hasn't itemid
        if (!$userinfo) {
            $eln->annotate_files('mod_eln', 'template', 'id');
        }
        $version->annotate_files('mod_eln', 'attachment', 'id');
        $version->annotate_files('mod_eln', 'content', 'id');

        // Return the root element (wiki), wrapped into standard activity structure
        return $this->prepare_activity_structure($eln);
