...
/**
 * Structure step to restore one eln activity
 */
class restore_eln_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('eln', '/activity/eln');
        $paths[] = new restore_path_element('eln_subwiki', '/activity/eln/subs/subwiki');
        $paths[] = new restore_path_element('eln_page', '/activity/eln/subs/subwiki/pages/page');
        $paths[] = new restore_path_element('eln_version', '/activity/eln/subs/subwiki/pages/page/versions/version');
        $paths[] = new restore_path_element('eln_annotation', '/activity/eln/subs/subwiki/pages/page/annotations/annotation');
        $paths[] = new restore_path_element('eln_link', '/activity/eln/subs/subwiki/pages/page/versions/version/links/link');
    ...
    protected function process_eln($data) {
        global $DB;
        ...
        // insert the eln record
        $newitemid = $DB->insert_record('eln', $data);
        // immediately after inserting "activity" record, call this
        $this->apply_activity_instance($newitemid);
        // Get OU Wiki id for later use.
        $this->elnid = $newitemid;
        $this->set_mapping('eln', $oldid, $newitemid);
    }

    protected function process_eln_subwiki($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->wikiid = $this->get_new_parentid('eln');
        ...
        $newitemid = $DB->insert_record('eln_subwikis', $data);
        $this->set_mapping('eln_subwiki', $oldid, $newitemid);
    }

    protected function process_eln_page($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->subwikiid = $this->get_new_parentid('eln_subwiki');

        $newitemid = $DB->insert_record('eln_pages', $data);

        $this->set_mapping('eln_page', $oldid, $newitemid);

        // Flush out any unsaved versions.
        $this->flush_versions();
    }

    protected function process_eln_version($data) {
        global $CFG;

        $data = (object)$data;
        $oldid = $data->id;

        $data->pageid = $this->get_new_parentid('eln_page');

        if (!isset($data->wordcount)) {
            // calculate the wordcount if it doesn't exist
            require_once($CFG->dirroot.'/mod/eln/locallib.php');
            $wordcount = eln_count_words($data->xhtml);
        ...
        // it from the process_eln_link function.
        $data->links = array();
    ...
    private function flush_versions() {
        ...
        foreach ($this->versions as $data) {
            // Insert version.
            $oldid = $data->id;
            $newversionid = $DB->insert_record('eln_versions', $data);
            $this->set_mapping('eln_version', $oldid, $newversionid, true);
    ...
    protected function process_eln_link($data) {
        $data = (object)$data;
    ...
    protected function process_eln_annotation($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('eln_annotations', $data);
    ...
    protected function after_execute() {
        global $DB, $CFG;
        $transaction = $DB->start_delegated_transaction();
        $elnid = $this->get_task()->get_activityid();

        // Flush out any unsaved versions.
        $this->flush_versions();

        // Add eln related files, no need to match by itemname (just internally handled context).
        $this->add_related_files('mod_eln', 'intro', null);
        $this->add_related_files('mod_eln', 'template', 'eln');

        // Add post related files.
        $this->add_related_files('mod_eln', 'attachment', 'eln_version');
        $this->add_related_files('mod_eln', 'content', 'eln_version');
        ...
        $sql = 'SELECT v.pageid,
                ...
                FROM
                    {eln} o
                    JOIN {eln_subwikis} s ON s.wikiid = o.id
                    JOIN {eln_pages} p ON p.subwikiid = s.id
                    JOIN {eln_versions} v ON v.pageid = p.id
                WHERE
                    o.id = ?
                GROUP BY v.pageid, p.id
                ORDER BY v.pageid';
        $rs = $DB->get_recordset_sql($sql, array($elnid));
        foreach ($rs as $entry) {
        ...
        $sql = 'SELECT l.id AS linkid, l.topageid
                FROM
                    ...
                    JOIN {eln_pages} p ON p.subwikiid = s.id
                    JOIN {eln_versions} v ON v.pageid = p.id
                    JOIN {eln_links} l ON l.fromversionid = v.id
                WHERE
                    o.id = ? AND l.topageid IS NOT NULL';
        $rs = $DB->get_recordset_sql($sql, array($elnid));
        $errors = array();
        foreach ($rs as $entry) {
            $newpageid = $this->get_mappingid('eln_page', $entry->topageid, null);
            ...
            $DB->set_field('eln_links', 'topageid', $newpageid, array('id' => $entry->linkid));
        }
        $rs->close();

        // Update xhtml field with correct annotations.
        // Get all table entries in eln_versions table for this wiki.
        ...
        foreach ($rs as $entry) {
            ...
            if ($found) {
                ...
                $DB->set_field('eln_versions', 'xhtml', $entry->xhtml, array('id' => $entry->id));
            }
        }
        ...
        if ($this->get_setting_value('userinfo') && eln_search_installed()) {
            eln_ousearch_update_all(false, $this->get_courseid());
        }
    }
}
