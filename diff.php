...
$url = new moodle_url('/mod/eln/diff.php', array('id' => $id, 'v1' => $v1, 'v2' => $v2, 'page' => $pagename));
$PAGE->set_url($url);
...
$pageversion1 = eln_get_page_version($subwiki, $pagename, $v1);
$pageversion2 = eln_get_page_version($subwiki, $pagename, $v2);
...
echo $elnoutput->eln_print_start($eln, $cm, $course, $subwiki, $pagename, $context,
    array(
        ...
        array('name' => get_string('changesnav', 'eln'), 'link' => null)),
    true, true
);

// Obtain difference between two versions
list($diff1, $diff2, $numchanges) = eln_diff_html($pageversion1->xhtml, $pageversion2->xhtml);

$fs = get_file_storage();
$files1 = ($files1 = $fs->get_area_files($context->id, 'mod_eln', 'attachment', $pageversion1->versionid, "timemodified", false)) ? $files1 : null;
$files2 = ($files2 = $fs->get_area_files($context->id, 'mod_eln', 'attachment', $pageversion2->versionid, "timemodified", false)) ? $files2 : null;

list($attachdiff1, $attachdiff2, $attachnumchanges) = eln_diff_attachments($files1, $files2, $context->id,
...
if ($numchanges == 0) {
    $annotations = eln_get_annotations($pageversion2);
    if (count($annotations) === 0) {
        $advice = get_string('diff_nochanges', 'eln');
    } else {
        $advice = get_string('diff_someannotations', 'eln');
    }
} else {
    $advice = get_string('advice_diff', 'eln');
}

print get_accesshide(get_string('changedifferences', 'eln'), 'h1');
print '<p class="ouw_advice">'.
     $advice.' '.
     get_string('returntohistory', 'eln',
    'history.php?'.eln_display_wiki_parameters($pagename, $subwiki, $cm)).'</p>';

// Obtain pluginfile urls.
$pageversion1->xhtml = file_rewrite_pluginfile_urls($pageversion1->xhtml, 'pluginfile.php',
    $context->id, 'mod_eln', 'content', $pageversion1->versionid);
$pageversion2->xhtml = file_rewrite_pluginfile_urls($pageversion2->xhtml, 'pluginfile.php',
    $context->id, 'mod_eln', 'content', $pageversion2->versionid);

// Obtain difference between two versions
list($diff1, $diff2) = eln_diff_html($pageversion1->xhtml, $pageversion2->xhtml);

// To make it look like a user object
$pageversion1->id = $pageversion1->userid;
$v1name = eln_display_user($pageversion1, $course->id);
$pageversion2->id = $pageversion2->userid;
$v2name = eln_display_user($pageversion2, $course->id);

// Disply the two versions
$v1 = new StdClass;
$v1->version = get_string('olderversion', 'eln');
$v1->date = userdate($pageversion1->timecreated);
$v1->savedby = get_string('savedby', 'eln', $v1name);
$v1->content = $diff1;
$v1->attachments = $attachdiff1;

$v2 = new StdClass;
$v2->version = get_string('newerversion', 'eln');
$v2->date = userdate($pageversion2->timecreated);
$v2->savedby = get_string('savedby', 'eln', $v2name);
$v2->content = $diff2;
$v2->attachments = $attachdiff2;

echo $elnoutput->eln_print_diff($v1, $v2);
