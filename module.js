...
M.mod_eln_view = {
    ...
    init : function(Y, args) {
        ...
        // or show annotations is disabled in eln settings
        ...
        // check to see whether there anno tags to show - either because there are none
        // or show annotations is disabled in eln settings
        var annospans = this.Y.all('span.eln-annotation-tag');
        if (annospans) {
            this.elnShowAllAnnotations("none");
    ...
    show_annotation_icons : function(show) {
        ...
        var container = this.Y.one('.eln-content');
        var hideclass = 'eln-hide-annotations';
        if (show) {
    ...
    elnToggleFunction : function(target, link) {
        return function() {
    ...
    elnShowFormFunction : function(target, header, link) {
        return function() {
    ...
    elnSetFields : function() {
        ...
        pagename.value = M.str.eln.typeinpagename;
...
//add by CRL
    ...
    elnResetThisField : function(field) {
        if (field.notusedyet) {
            field.value = '';
    ...
    elnShowAllAnnotations : function(action) {
        annoboxes = this.Y.all('span.eln-annotation');
        annoboxes.each(function (annobox) {
            ...
            if (action == "block") {
                imgtag.set('alt', M.str.eln.collapseannotation);
                imgtag.set('title', M.str.eln.collapseannotation);
            } else if (action == "none") {
                imgtag.set('alt', M.str.eln.expandannotation);
                imgtag.set('title', M.str.eln.expandannotation);
            }
        });

        if(action == "block") {
            this.elnSwapAnnotationUrl("hide");
        } else if(action == "none") {
            this.elnSwapAnnotationUrl("show");
        }
    },

    elnSwapAnnotationUrl : function(action){
    ...
    elnShowHideAnnotation : function(id) {
        ...
        if (box.style.display == "block") {
            ...
            this.elnSwapAnnotationUrl("show");
            imgtag.alt = M.str.eln.expandannotation;
            imgtag.title = M.str.eln.expandannotation;
        } else {
            box.style.display = "block";
            imgtag.alt = M.str.eln.collapseannotation;
            imgtag.title = M.str.eln.collapseannotation;
            annoboxes = this.Y.all('span.eln-annotation');
    ...
    setupspans : function(span) {
        ...
        span.on('keydown', function(e) {
            ...
            if (evt.keyCode == 13 || evt.keyCode == 32) {
                _this.elnShowHideAnnotation("annotationbox" + span.get('id').substring(10));
...
M.mod_eln_annotate = {
    ...
    init : function(Y, args) {
        ...
        var _this = this;
        var save = M.util.get_string('add', 'eln');
        var cancel = M.util.get_string('cancel', 'eln');
        ...
        // setup keycodes
        var markers = this.Y.all('span.eln-annotation-marker');
        ...
        init : function(Y, args) {
            ...
            btns.on('click', function(e) {
                function savefail() {
                    ...
                    var content = M.util.get_string('savefailnetwork', 'eln');
                    var panel = new M.core.dialogue({
                        headerContent: M.util.get_string('savefailtitle', 'eln'),
                        bodyContent: content,
                        width: 400,
                        centered: true,
                        render: true,
                    ...
                    cancel.on('click', function(e) {
                        var form = Y.one('#eln_belowtabs #mform1');
